...
namespace ProcessesApi.V1.UseCase
{
    public class SoleToJointService : ISoleToJointService
    {
        ...
        private void SetUpStateActions()
        {
            ...
            Configure(SoleToJointStates.BreachChecksPassed, Assignment.Create("tenants"), null);
            ...
            ConfigureAsync(SoleToJointStates.DocumentsRequestedAppointment, Assignment.Create("tenants"), OnDocumentsRequestedAppointment);
        ...
        private async Task OnAutomatedCheckFailed(UpdateProcessState processRequest)
        {
            ...
            await PublishEvent(
                ProcessEventConstants.PROCESS_CLOSED_EVENT,
                $"{_soleToJointProcess.ProcessName} closed: Automatic Eligibility Checks failed");
        }
        ...
        private async Task OnManualCheckFailed(UpdateProcessState processRequest)
        {
            await PublishEvent(
                ProcessEventConstants.PROCESS_CLOSED_EVENT,
                $"{_soleToJointProcess.ProcessName} closed: Manual Eligibility Checks failed");
        }
        ...
        private async Task OnTenancyBreachCheckFailed(UpdateProcessState processRequest)
        {
            await PublishEvent(
                ProcessEventConstants.PROCESS_CLOSED_EVENT,
                $"{_soleToJointProcess.ProcessName} closed: Breach of Tenancy Checks failed");
        }

        private async Task OnDocumentsRequestedDes(UpdateProcessState processRequest)
        {
            await PublishEvent(
                ProcessEventConstants.PROCESS_UPDATED_EVENT,
                $"{_soleToJointProcess.ProcessName}: Supporting Documents requested through the Document Evidence Store.");
        }

        private async Task OnDocumentsRequestedAppointment(UpdateProcessState processRequest)
        {
            var appointmentDateTime = GetAppointmentDateTime(processRequest.FormData);

            await PublishEvent(
                ProcessEventConstants.PROCESS_UPDATED_EVENT,
                new RequestAppointmentEventData
                {
                    DateTime = appointmentDateTime.ToIsoString(),
                    Description = $"{_soleToJointProcess.ProcessName}: Supporting Documents requested via an office appointment on {appointmentDateTime:f}"
                });
        }

        private async Task OnDocumentsAppointmentRescheduled(UpdateProcessState processRequest)
        {
            var oldAppointmentDateTime = GetAppointmentDateTime(_soleToJointProcess.CurrentState.ProcessData.FormData);
            ...
            await PublishEvent(
                    ...
                    new RequestAppointmentEventData
                    {
                        DateTime = newAppointmentDateTime.ToIsoString(),
                        Description = $"{_soleToJointProcess.ProcessName}: Supporting Documents office appointment changed to {newAppointmentDateTime:f}"
                    },
                    oldAppointmentDateTime.ToUniversalTime().ToIsoString())
                .ConfigureAwait(false);
        }
        ...
        private async Task PublishEvent(string eventType, object newData, object oldData = null)
        {
            var snsMessage = _snsFactory.Create(
            ...
            await _snsGateway.Publish(snsMessage, processTopicArn).ConfigureAwait(false);
        }
