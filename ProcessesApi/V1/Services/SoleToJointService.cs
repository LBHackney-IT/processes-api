...
namespace ProcessesApi.V1.Services
{
    public class SoleToJointService : ProcessService, ISoleToJointService
    {
        ...
        private async Task CheckAutomatedEligibility(StateMachine<string, string>.Transition transition)
        {
            ...
            SharedHelper.ValidateFormData(formData, new List<string>() { SoleToJointKeys.IncomingTenantId, SoleToJointKeys.TenantId });
        ...
        {
            var processRequest = x.Parameters[0] as ProcessTrigger;
            SharedHelper.ValidateFormData(processRequest.FormData, new List<string>() { SharedKeys.Comment });

            _eventData = SharedHelper.CreateEventData(processRequest.FormData, new List<string> { SharedKeys.Comment });
            await PublishProcessClosedEvent(x).ConfigureAwait(false);
        }
        ...
        public void AddAppointmentDateTimeToEvent(Stateless.StateMachine<string, string>.Transition transition)
        {
            ...
            SharedHelper.ValidateFormData(trigger.FormData, new List<string>() { SoleToJointKeys.AppointmentDateTime });

            _eventData = SharedHelper.CreateEventData(trigger.FormData, new List<string> { SoleToJointKeys.AppointmentDateTime });
        }
